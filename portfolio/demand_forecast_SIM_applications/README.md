...
`src/make_sim_applications_csv.py` にて生成しています。
...
前月ベースラインに対して MAE をほぼ半分（361 → 189）に減らした。
...
## AIとの分担・自分がやったこと
実装（コード）はChatGPTとCodexに任せた。自分が決めたのは次の4つ。
- 擬似データの月効果・曜日効果の数値（3月が最多、6月に落ちる、土日が多い）。携帯売り場の実感を数値にした
- 何と比べるか。「前月と同じ」「3か月平均」に勝てないモデルは採用しない、と先に決めた
- テスト期間（直近3・6・12か月）を変えて安定性を見ること
- 結果の読み方と、限界（擬似データ・外部要因なし）を書くこと
