...
## AIとの分担・自分がやったこと
実装（前処理・学習・API・Docker）はChatGPTとCodexに任せた。自分が決めたのは次の4つ。
- データ。店頭で実際に聞かれる質問を元に自分で文を作り、自分でラベルを付けた。境界が曖昧な質問（MNPの話に見えて主な関心は料金）をわざと入れた
- カテゴリの定義。「質問を聞いた瞬間にどの説明フローへ進むか」という店頭の判断をそのまま3分類にした
- 確信度が低いときは機械が決めず人に戻す設計（`needs_review`）と、その閾値0.65
- APIが返す項目（ラベル・確信度・人確認フラグ・上位3候補）

コードを1行ずつ説明することはできないが、上の設計理由と流れは説明できる。
